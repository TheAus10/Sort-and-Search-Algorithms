...
{  
    double key;		// hodls value from array being checked
	int n;			// used to hold array index
	
	// shifts indexes with a value greater than key to one position back
    for (int i = 1; i <= h; i++) 
    {  
        key = list[i];  
        n = i - 1;  
  
        while (n >= 0 && list[n] > key) 
        {  
            list[n + 1] = list[n];  
            n -= 1;  
        }  
